...
local AA_SECTION = MISC_TAB:AddSection({
	Name = "- Ability Abuse -"
})
...
MISC_TAB:AddToggle({
	...
	Default = false,
	Callback = function(Value)
		getgenv().zahando = Value

		while task.wait() and getgenv().zahando == true do
			game:GetService("ReplicatedStorage").ZaHandoPull:FireServer()
		end
	end    
})

MISC_TAB:AddToggle({
	Name = "Spam Silver Ball (KS:E)",
	Default = false,
	Callback = function(Value)
		getgenv().silver = Value

		while task.wait() and getgenv().silver == true do
			game:GetService("ReplicatedStorage").SilverBall:FireServer()
		end
	end    
})

MISC_TAB:AddToggle({
	Name = "Edgelord Push (KS)",
	Default = false,
	Callback = function(Value)
		game:GetService("ReplicatedStorage").PushRemote:FireServer()
	end    
})

MISC_TAB:AddToggle({
	Name = "Spam Last Breath",
	Default = false,
	Callback = function(Value)
		getgenv().lastbreath = Value

		while task.wait() and getgenv().lastbreath == true do
			game:GetService("ReplicatedStorage"):WaitForChild("funniability"):FireServer()
		end
	end    
})
...
CREDITS_TAB:AddLabel("Owner - Zyer, RoShowcaser & ScriptMaster")
CREDITS_TAB:AddLabel("Most Ability Spams - @yofetass")

-- notification

OrionLib:MakeNotification({
	Name = "Successfully Executed RSZ Hub",
	Content = "Successfully Executed RSZ Hub in "..gamename.."!",
